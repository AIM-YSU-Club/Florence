		...
		<div>
			...
			{!loading &&
				selectedAtc.map((code) => {
					...
					const climateCharts = [
						{ key: 'ta', label: '평균 기온', unit: '°C', color: '#5b9bd5', data: result.ta_4w },
						{ key: 'hm', label: '평균 습도', unit: '%', color: '#6ba5c7', data: result.hm_4w },
						{ key: 'rn', label: '강수량', unit: 'mm', color: '#6ba589', data: result.rn_4w },
					];

					return (
						<div key={code} className="mb-10 rounded-2xl border border-(--border) bg-(--card) p-6 shadow-(--shadow-sm)">
							...
							<div className="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
								<StatCard label="예상 사용량 (4주)" value={formatNumber(result.predicted_value)} unit="건" accent />
								<StatCard label="3년 평균 사용량" value={formatNumber(result.mean_value)} unit="건" />
								<StatCard
									label="평균 대비 증감률"
									...
									return (
										<div key={chart.key} className="rounded-xl border border-(--border) bg-(--bg) p-4">
											...
											<ResponsiveContainer width="100%" height={140}>
												<AreaChart
													data={chart.data.map((value, index) => ({ name: weekLabels[index], value }))}
			...
			{!loading && !selectedAtc.length && <p className="py-16 text-center text-sm text-(--text-muted)">위 필터에서 ATC 코드를 선택하세요.</p>}
		</div>
	);
}
