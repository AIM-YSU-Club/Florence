				...
				{medicines.map((medicine, index) => {
					...
					return (
						<div
							...
							style={{ background: `linear-gradient(145deg, ${palette.from}, ${palette.to})`, minHeight: '200px' }}
						>
							<button
								type="button"
								onClick={(e) => {
							...
							<button type="button" onClick={() => onCardClick(medicine)} className="flex h-full w-full flex-col justify-between p-5 text-left text-white">
								...
								<div className="relative z-10 mt-auto">
									<p className="text-lg font-black leading-tight">{medicine.name}</p>
									<div className="mt-2">
										<PtStatusBadge status={medicine.pt_status} variant="dark" />
									</div>
								</div>
							</button>
						</div>
