...
export const Header = () => {
	...
	return (
		<header className="relative flex h-16 items-center justify-between border-b border-(--border) bg-(--card)/80 px-6 backdrop-blur-md">
			<Link to="/" className="flex items-center gap-2.5" reloadDocument={true}>
				...
				<span className="text-lg font-black tracking-tight text-(--text-h)">Florence</span>
				<span className="rounded-md bg-(--quaternary) px-1.5 py-0.5 text-[10px] font-bold text-(--text-muted)">BETA</span>
			</Link>
				...
				{loggedIn ? (
					<>
						<div className="flex items-center gap-2 rounded-lg bg-(--bg-2)/50 px-3 py-1.5">
							...
							<span className="text-xs font-semibold text-(--text-muted)">연결됨</span>
						</div>
