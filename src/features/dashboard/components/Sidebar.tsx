...
function SidebarItem({
	...
	return (
		<NavLink to={to} end={end}>
			{({ isActive }) => (
				<div
					className={`group relative flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-[13px] font-semibold transition-all duration-150 ${isActive ? 'bg-(--primary)/12 text-(--primary)' : 'text-(--text) hover:bg-(--bg)/80'}`}>
					{isActive && (
						<span className="absolute top-1/2 left-0 h-5 w-0.75 -translate-y-1/2 rounded-r-full bg-(--primary)" />
					)}
					<span
						className={`flex h-7 w-7 items-center justify-center rounded-md transition-all ${isActive ? 'bg-(--primary) text-white shadow-sm' : 'bg-(--bg-2)/60 text-(--text-muted) group-hover:text-(--text)'}`}>
						{icon}
					</span>
					<span className="flex-1 text-left">{label}</span>
					{badge !== undefined && (
						<span
							className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${isActive ? 'bg-(--primary)/20 text-(--primary)' : 'bg-(--bg-2)/60 text-(--text-muted)'}`}>
							{badge}
						</span>
					)}
				</div>
